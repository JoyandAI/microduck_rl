...
现有 `microduck_rl/scripts/record_pendulum_bench.py` 是给 HL-2909 写的，跑 HD-1910 前
**必须**按真机行为改 4 处（均已实测验证，改法见 `hd1910_servo_notes.md` §6.3）：
...
| 2 | 配置段**不要写 reg33=0**；保持 `33=4`（纯位置PD/Sim2Real，出厂默认） | 写 0 会切到模式 0（角度伺服+限力），与部署控制律不一致，识别结果不能用 |
| 3 | 目标写入做**环绕感知**，或保证 `q_zero` 远离编码器 0/4095 | mode 4 **有效目标被钳位 [0,4095] 且误差不回绕**：q_zero 靠近 0/4095 时小目标会冲向限位（实测 4094→338 走 -3755 LSB 长路径；写 6349 停在 4094） |
...
改完先用 `tools/check_servo_bench.py --port /dev/ttyACM0 --id 14 --yes` 复检到 **PASS**，
再进第 2 节。**在此适配完成前，禁止上摆锤采集。**
...
| 参数 | 含义 | 怎么量 | 示例 | 你的实测值 |
|---|---|---|---|---|
| `m_arm` | 摆杆质量（只称杆） | 电子秤 0.1 g | 0.018 kg | `____` |
